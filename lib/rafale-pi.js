...
//      hit, rang, etaMin, etaAtMs, gustKmh,
//      picKmh, picRang, picAtMs, picDansMin, baseKmh, sautKmh,
...
const DEFAUTS = Object.freeze({
  seuilKmh: 40,         // ⛔ arbitrage Yann 09/09 : « ça devient sérieux »
                        //    pour du parapente. 11,11 m/s dans les octets.
...
// ══════════════════════════════════════════════════════════════════════
function refus(carte, nowMs, raison, seuilKmh, extra) {
...
function rafalePi(carte, lat, lon, nowMs, opts) {
  ...
  if (!carte) return refus(null, nowMs, 'indisponible', o.seuilKmh);
  const ageMin = (nowMs - carte.runMs) / 60_000;
  if (ageMin > o.ageRefusMin) return refus(carte, nowMs, 'run-trop-vieux', o.seuilKmh);
  if (!(lat != null && lon != null) || !carte.dansEmprise(lat, lon)) return refus(carte, nowMs, 'hors-emprise', o.seuilKmh);
  ...
  return {
    ...
    // ⚠️ Le SAUT, et pas seulement le pic. 55 km/h un jour de tramontane
    // à 50 n'est pas un événement ; 55 sur un fond à 12, si. Le push s'en
    // sert pour choisir sa phrase, jamais pour décider seul — le seuil
    // absolu reste l'arbitrage de Yann.
    sautKmh: (picKmh == null || baseKmh == null) ? null : Math.round(picKmh - baseKmh),
