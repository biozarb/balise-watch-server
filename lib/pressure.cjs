...
//   src/lib/phenomena.ts  KINDS  sha256:f0750672fe36bddd
//   src/lib/phenomena.ts  SIGNS  sha256:b4983adf6eca7e6c
//   src/lib/phenomena.ts  phenomenonFromRow  sha256:aeb15d6a77cde0ef
//   src/lib/phenomena.ts  pickStationFor  sha256:31f36ca192cb9abc
//   src/lib/phenomena.ts  resolveAnchors  sha256:fb91b428c29b9153
//   src/lib/phenomena.ts  effectiveThreshold  sha256:cb62b4ef56ebde56
//   src/lib/phenomena.ts  signMatches  sha256:d99645e3d7e79752
//   src/lib/phenomena.ts  phenomenonLevel  sha256:39a0f59ba74f37c8
//   src/lib/phenomena.ts  phenomenonDirection  sha256:75dd99b8c837b7a1
// ══════════════════════════════════════════════════════════════════
...
// ─── src/lib/phenomena.ts → KINDS ───
const KINDS                    = ['crest_foehn', 'gap_wind', 'plain_gradient'];

// ─── src/lib/phenomena.ts → SIGNS ───
const SIGNS                    = ['both', 'neg', 'pos'];

// ─── src/lib/phenomena.ts → phenomenonFromRow ───
function phenomenonFromRow(r               )                 {
  return {
    id: r.id,
    userId: r.user_id,
    kind: (KINDS.includes(r.kind) ? r.kind : 'crest_foehn')                  ,
    label: r.label,
    aName: r.a_name, aLat: r.a_lat, aLon: r.a_lon,
    bName: r.b_name, bLat: r.b_lat, bLon: r.b_lon,
    stationA: r.station_a, stationB: r.station_b,
    gapName: r.gap_name, gapLat: r.gap_lat, gapLon: r.gap_lon, gapAlt: r.gap_alt,
    bearing: r.bearing,
    // Replis = les constantes historiques, au cas où la migration ne
    // serait pas passée sur un environnement (colonnes absentes → null).
    thresholdHpa: r.threshold_hpa ?? 4,
    thresholdStrongHpa: r.threshold_strong_hpa ?? 8,
    activeSign: (SIGNS.includes(r.active_sign) ? r.active_sign : 'both')              ,
    spanKm: r.span_km,
    notes: r.notes,
    // Un tableau vide vaut « rien à dire » : on le ramène à null pour
    // que l'affichage n'ait qu'un seul cas d'absence à traiter.
    localNames: Array.isArray(r.local_names) && r.local_names.length
      ? r.local_names.filter(n => !!n && n.trim() !== '')
      : null,
  };
}
...
// ─── src/lib/phenomena.ts → effectiveThreshold ───
/** Le seuil qui s'applique : surcharge du compte, sinon celui du phénomène. */
function effectiveThreshold(ph                , userOverride                )         {
  return userOverride ?? ph.thresholdHpa;
}

// ─── src/lib/phenomena.ts → signMatches ───
/**
 * Le signe du Δ correspond-il à ce que ce phénomène surveille ?
 * Un Nordföhn ne se déclenche pas sur un Δ de foehn du sud, même si sa
 * valeur absolue franchit le seuil : c'est le même axe, pas le même vent.
 */
function signMatches(delta        , ph                )          {
  if (ph.activeSign === 'both') return true;
  if (ph.activeSign === 'neg') return delta < 0;
  return delta > 0;
}

// ─── src/lib/phenomena.ts → phenomenonLevel ───
function phenomenonLevel(
  delta        ,
  ph                ,
  userOverride                ,
)                  {
  if (!signMatches(delta, ph)) return 0;
  const mag = Math.abs(delta);
  if (mag >= ph.thresholdStrongHpa) return 3;
  if (mag >= effectiveThreshold(ph, userOverride)) return 2;
  return 0;
}

// ─── src/lib/phenomena.ts → phenomenonDirection ───
/**
 * Vers quel versant. L'air va de la haute vers la basse pression et
 * redescend côté basse : Δ = A − B < 0 → B plus haut → vers A.
 * Générique : vrai pour n'importe quelle orientation d'axe, l'UI affiche
 * le nom de l'extrémité concernée, jamais « nord/sud » en dur.
 */
function phenomenonDirection(
  delta        ,
  ph                ,
  userOverride                ,
)                      {
  if (phenomenonLevel(delta, ph, userOverride) === 0) return 'none';
  return delta < 0 ? 'toA' : 'toB';
}

module.exports = { QFF_CONVERSION_UNCERTAINTY_HPA, QFF_NATIVE_UNCERTAINTY_HPA, PRESSURE_MAX_ALT, PRESSURE_WARN_ALT, PRESSURE_MAX_KM, QNH_DISTANCE_PENALTY, qnhToStation, stationToQff, qnhToQff, normalizePressure, pressureDelta, smoothPressureSeries, readingFromQff, readingFromMetar, readingFromStation, deltaSeries, buildPressureReferential, haversineKm, phenomenonFromRow, pickStationFor, resolveAnchors, effectiveThreshold, signMatches, phenomenonLevel, phenomenonDirection };
