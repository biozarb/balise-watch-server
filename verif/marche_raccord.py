...
def mesurer(col, man, crier=print):
    i0 = {p["nom"]: k for k, p in enumerate(PARAMS_0025)}
    i1 = {p["nom"]: k for k, p in enumerate(PARAMS_001)}
    c0 = np.asarray(col.c0025, dtype=np.float32)
    c1 = np.asarray(col.c001, dtype=np.float32)

    resultats = {}
    crier(f"── MARCHE AU RACCORD 0,01° / 0,025°, run {col.run} ──")
    crier(f"   {len(col.balises)} balises × {len(col.steps)} échéances\n")
    ...
    for niveau in NIVEAUX_H_001:
        ...
        # On n'a de sens à parler d'angle que si le vent existe.
        fort = np.hypot(u0[bon], v0[bon]) > 1.0
        qv, qa = quantiles(d), quantiles(da[fort]) if fort.any() else None
        resultats[niveau] = dict(vitesse=qv, angle=qa,
                                 n_pour_angle=int(fort.sum()))
    ...
    dz = [b["z_0025"] - b["z_001"] for b in col.balises
    ...
    return dict(run=col.run, par_niveau=resultats, orographie=qz,
                n_balises=len(col.balises), n_echeances=len(col.steps))
